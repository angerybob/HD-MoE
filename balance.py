...
optimizer = MoE3DPNMOptimizer(E=64, SE=0,h=2048,D=32, B=batch,BW=50e9, comp=5e12,routing_trace=data)
...
file_path = f"/data/home/haochenhuang/deployment/results/reasoning_ds_5.0_TFLOPS_50.0_GBPS_for_4*8_mesh_128_batches/arrays_5.0_TFLOPS_50.0_GBPS_in_layer_{layer_id}.npz"
...
def load(P,optimizer,layer_id):
    ...
    #load=compute_load/optimizer.comp
...
load_EP=load(P_ep,optimizer,layer_id)*1e6
load_ours=load(P,optimizer,layer_id)*1e6
#pdb.set_trace()
mesh_shape=(4,8)
...
vmin = min(np.min(load_EP), np.min(load_ours))
vmax = max(np.max(load_EP), np.max(load_ours))
# 创建一个包含两个子图的画布
fig, axes = plt.subplots(1, 2, figsize=(15, 4))
heatmap=sns.heatmap(load_EP, ax=axes[0], cmap="YlGnBu", vmin=vmin, vmax=vmax, annot=False)
...
# 设置颜色条主标签字体大小
cbar.set_label('Workload (us)', fontsize=24)  # 主标签字体大小

# 设置颜色条刻度标签字体大小
cbar.ax.tick_params(labelsize=24)  # 刻度标签字体大小
size=30
axes[0].set_title("EP Load", fontsize=size)
axes[0].set_xlabel("X", fontsize=size)
axes[0].set_ylabel("Y", fontsize=size)

# 绘制垂直链路热力图
heatmap=sns.heatmap(load_ours, ax=axes[1], cmap="YlGnBu", vmin=vmin, vmax=vmax, annot=False)
...
axes[1].set_title("HD-MoE Load", fontsize=size)
axes[1].set_xlabel("X", fontsize=size)
axes[1].set_ylabel("Y", fontsize=size)

# 显示图形
plt.tight_layout()
plt.savefig('/data/home/haochenhuang/deployment/load_imbalance.png')
plt.close()

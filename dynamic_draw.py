...
# 数据加载
with open("/data/home/haochenhuang/deployment/result_dynamic.json", "r") as file:
    data = json.load(file)
...
for entry in data:
    config = entry['config']
    if config["model"] == "ds" and config["comp_TFLOPS"] == 5.0 and config["BW_GBPS"] == 50.0:
...
# 显示图形
plt.tight_layout()
plt.savefig("/data/home/haochenhuang/deployment/dynamic_deployment_latency_speedup.pdf")

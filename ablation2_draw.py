...
for entry in data:
    ...
    if model in models:
        ...
        ep_latency = entry['EP']['computation_us']

        link_balancing_latency = entry['node_balancing']['computation_us']
        if mesh_shape==(4,8):
        ...
        if mesh_shape==(4,8):
            ...
            speedup_data[model][hardware_tuple] = {
                'speedup_EP': ep_latency/link_balancing_latency 
            }
        if hardware_tuple==(5.0,50.0):  
...
ax.bar(x, [latency_data[model][(hardware["comp_TFLOPS"], hardware["BW_GBPS"])]['EP'] for hardware in hardware_configs], width, label='EP', color=colors[0], edgecolor="black")
...
ax.bar(x + width, [latency_data[model][(hardware["comp_TFLOPS"], hardware["BW_GBPS"])]['node_balance'] for hardware in hardware_configs], width, label='node_balance', color=colors[1], edgecolor="black")
...
ax2.plot(x + width, [speedup[(hardware["comp_TFLOPS"], hardware["BW_GBPS"])]['speedup_EP'] for hardware in hardware_configs], color=colors[0], linestyle="--", marker="s", label="Speedup EP", markersize=8)
...
ax.bar(x, [latency_data1[model][mesh_shape]['EP'] for mesh_shape in mesh_shapes], width, label='EP', color=colors[0], edgecolor="black")
...
ax.bar(x + width, [latency_data1[model][mesh_shape]['node_balance'] for mesh_shape in mesh_shapes], width, label='node_balance', color=colors[1], edgecolor="black")
...
ax2.plot(x + width, [speedup[mesh_shape]['speedup_EP'] for mesh_shape in mesh_shapes], color=colors[0], linestyle="--", marker="s", label="Speedup EP", markersize=8)
...
# 显示图形
plt.tight_layout()
plt.savefig("/data/home/haochenhuang/deployment/ablation_study_node_comp_balancing_speedup.pdf")

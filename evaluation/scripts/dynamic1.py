...
optimizer = MoE3DPNMOptimizer(E=E,e=e, SE=SE,h=h,IS=IS,B=batch, D=D,BW=50*1e9, comp=5*1e12, num_layers=num_layers,mlp_first=mlp_first,routing_trace=data)
...
result = {
    ...
    "pre_broadcasting": {
        ...
        "speedup_vs_TP": round((tp_comp_dynamic + tp_comm_dynamic) / (comp_pre + comm_pre), 2),
        "speedup_vs_EP": round((ep_comp_dynamic + ep_comm_dynamic) / (comp_pre + comm_pre), 2),
        "speedup_vs_link_balancing": round((comp_dynamic + comm_link_dynamic) / (comp_pre + comm_pre), 2)
    }
}


# 3. 文件写入逻辑
file_path = "evaluation/results/result3_dynamic.json" # 您可以根据需要更改文件名
new_data = result
...
if os.path.exists(file_path):
    with open(file_path, "r") as f:
        try:
            ...
            if not isinstance(old_data, list): # 确保旧数据是列表
                old_data = [old_data]
        except json.JSONDecodeError:
            old_data = [] # 如果文件为空或损坏，则创建新列表
...
print(f"\nResults have been successfully saved to {file_path}")